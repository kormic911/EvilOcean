...
public class EventSpawnPlayer {
	...
	public void onPlayerLoggedInEvent(PlayerLoggedInEvent event) {
		if((event.player instanceof EntityPlayerMP) && !event.player.world.isRemote) {
			...
			if(player.ticksExisted < 3 && !persist.getBoolean(TAG_MADE_RAFT) && event.player.world.provider.getDimension() == 0) {
	...
	private void adjustPlayerSpawn(EntityPlayerMP player) {
		...
		if(player != null) {
			...
			if(test || !persist.getBoolean(TAG_HAS_OWN_RAFT)) {
				this.buildSpawn(player.world, pos.getX(), pos.getY(), pos.getZ());
